import uvicorn
import httpx
from fastapi import FastAPI, HTTPException
from pydantic import BaseModel
from aiogram import Bot
from aiogram.types import ForumTopic
from typing import Optional

# --- КОНФИГУРАЦИЯ ---
BOT_TOKEN = "8229314742:AAHM35Yx6_t8C6qfIvALcckdO9hFqQOKpBw"
EXTERNAL_API_URL = "https://form2.tethertrc20.ru/api/values"

# Маппинг: НАЗВАНИЕ города из API -> ID группы в Telegram
# Важно: Названия должны в точности совпадать с тем, что возвращает API (поле NAME)
CITIES_TO_GROUPS = {
    "Владимир": 0,
    "Екатеринбург": -1003834359521,
    "Иваново": -1003409849410,
    "Казань": 0,
    "Кострома": -1003749359451,
    "Москва": -1003559739114,
    "Нижний Новгород": -1003731754411,
    "Нижний Тагил": -1003659046288,
    "Новосибирск": -1003760499721,
    "Омск": -1003742180272,
    "Пермь": -1003849401068,
    "Ростов-на-Дону": -1003837153559,
    "Рязань": 0,
    "Самара": -1003809968038,
    "Санкт-Петербург": -1003766727039,
    "Сочи": -1003822120037,
    "Сургут": -1003812933026,
    "Тверь": -1003743410590,
    "Тольятти": -1003836081700,
    "Тула": -1003770447273,
    "Тюмень": -1003814406575,
    "Уфа": -1003793984695,
    "Челябинск": -1003600530409,
    "Ярославль": -1003721184896
}

class TransactionData(BaseModel):
    city_id: int
    brand_id: Optional[int] = None  # Добавили ID бренда для поиска партнера
    creator_id: int
    visit_time: str
    transaction_type: str
    client_full_name: str
    amount: float
    currency: str
    wallet_address: str
    network: str
    wallet_owner_type: str
    form_url: str

app = FastAPI()
bot = Bot(token=BOT_TOKEN)

def format_message(data: TransactionData, city_name: str, partner_name: str) -> str:
    """Форматирование сообщения для Telegram"""
    
    type_text = "<b>ПРЯМАЯ</b>" if data.transaction_type == "direct" else \
                "<b>ОБРАТНАЯ</b>" if data.transaction_type == "reverse" else data.transaction_type

    wallet_owner_type_text = "Клиентский" if data.wallet_owner_type == "client" else \
                             "Партнёрский" if data.wallet_owner_type == "partner" else data.wallet_owner_type

    return (
        f"🔄 <b>Тип сделки:</b> {type_text}\n"
        f"🏛 <b>Город:</b> {city_name}\n"
        f"🤝 <b>Чья сделка:</b> {partner_name}\n\n"
        f"👤 <b>Клиент:</b> {data.client_full_name}\n"
        f"💰 <b>Сумма:</b> {data.amount} {data.currency}\n\n"
        f"🏦 <b>Кошелек:</b> <code>{data.wallet_address}</code>\n"
        f"🌐 <b>Сеть:</b> {data.network}\n"
        f"💰 <b>Тип кошелька:</b> {wallet_owner_type_text}\n\n"
        f"🕒 <b>Дата и время:</b> {data.visit_time}\n\n"
        f"🔗 <a href='{data.form_url}'>Ссылка на форму</a>"
    )

async def get_external_data():
    """Запрос к внешнему API для получения списка городов и партнеров"""
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(EXTERNAL_API_URL, timeout=10.0)
            response.raise_for_status()
            return response.json()
        except Exception as e:
            print(f"Ошибка при запросе к API: {e}")
            return None

@app.post("/new-transaction")
async def handle_transaction(data: TransactionData):
    # 1. Запрос к внешнему API
    api_values = await get_external_data()
    if not api_values:
        raise HTTPException(status_code=500, detail="Ошибка: Не удалось получить данные от внешнего API")

    # 2. Поиск города (ID города в число -> поиск названия)
    departments = api_values.get("DEPARTMENTS", [])
    city_name = "Неизвестный город"
    
    for d in departments:
        try:
            if int(d.get("ID")) == int(data.city_id):
                city_name = d.get("NAME")
                break
        except (ValueError, TypeError):
            continue

    # 3. Поиск партнера (Бренда)
    # Берем список PARTNERS, переводим ID в числа и сравниваем
    partners_list = api_values.get("PARTNERS", [])
    partner_name = "Неизвестный партнер"
    
    if data.brand_id is not None:
        for p in partners_list:
            try:
                # Преобразуем оба ID в int для надежности
                if int(p.get("ID")) == int(data.brand_id):
                    partner_name = p.get("NAME", "Имя не указано")
                    break
            except (ValueError, TypeError):
                # Если ID в API — не число, просто пропускаем этот элемент
                continue

    # 4. Проверка наличия группы в конфиге
    group_id = CITIES_TO_GROUPS.get(city_name)
    
    if not group_id:
        print(f"Ошибка: Город '{city_name}' найден в API, но не настроен в CITIES_TO_GROUPS")
        raise HTTPException(status_code=404, detail=f"Группа для города {city_name} не найдена в коде")

    try:
        # 5. Создание темы
        topic_title = f"{data.client_full_name} | {data.amount} {data.currency}"
        new_topic: ForumTopic = await bot.create_forum_topic(chat_id=group_id, name=topic_title)
        
        # 6. Отправка сообщения
        await bot.send_message(
            chat_id=group_id,
            message_thread_id=new_topic.message_thread_id,
            text=format_message(data, city_name, partner_name),
            parse_mode="HTML",
            disable_web_page_preview=True
        )

        return {
            "status": "success",
            "city": city_name,
            "partner": partner_name,
            "topic_id": new_topic.message_thread_id
        }

    except Exception as e:
        print(f"Ошибка Telegram: {e}")
        raise HTTPException(status_code=500, detail=str(e))

if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=4000)